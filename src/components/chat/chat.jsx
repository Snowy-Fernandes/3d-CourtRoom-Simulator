...
export const Chat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/api/groq", // Updated endpoint to match GROQ API
        {
          message: userInput, // This stays the same unless the GROQ API expects a different field name
        },
        {
          headers: {
      ...
      const generatedText = response.data.response; // Assuming the response structure from GROQ is the same
  ...
  const toggleInput = () => {
    ...
    // Stop voice recognition if it's currently listening
    if (recognitionRef.current) {
      recognitionRef.current.stop();
      setIsListening(false);
    }
